...
def generate_component_code(class_name, interface_name):
    ...
    component_code += f"\tselector: '{class_name.lower()}-detail',\n"
    component_code += f"\ttemplateUrl: '{class_name.lower()}.component.html',\n"
    component_code += f"\tstyleUrls: ['{class_name.lower()}.component.scss']\n"
    component_code += "})\n"
    component_code += f"export class {class_name}DetailComponent implements OnInit {{\n"
    ...
    component_code += f"\tget{class_name}(id: number) {{\n"
    component_code += f"\t\tthis.{class_name.lower()}Service.get{class_name}(id).subscribe(({class_name.lower()}) => {{\n"
    component_code += f"\t\t\tthis.name.setValue({class_name.lower()}.name);\n"
    component_code += f"\t\t\tthis.type.setValue({class_name.lower()}.type);\n"
    component_code += f"\t\t\tthis.effects.setValue({class_name.lower()}.effects);\n"
    component_code += "\t\t}});\n"
    component_code += "\t}\n\n"
    ...
    component_code += "\t\t};\n\n"
    component_code += f"\t\tthis.{class_name.lower()}Service.update{class_name}({class_name.lower()}).subscribe(updated{class_name} => {{\n"
    component_code += "\t\t\tconsole.log('EquipmentEntity updated successfully:', updatedEquipment);\n"
...
def generate_template_code(class_name):
    ...
    template_code += "\t<button mat-raised-button color=\"primary\" type=\"submit\">Save</button>\n"
    template_code += "</form>\n"
